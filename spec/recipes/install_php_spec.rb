...
describe 'ingenerator-php::install_php' do
  ...
  it "writes a common php.ini in /etc/php/7.4/php.ini" do
    expect(chef_run).to render_file('/etc/php/7.4/php.ini').with_content(/About php.ini/)
  end
  ...
  it "includes custom directives in the php.ini file" do
    ...
    expect(chef_run).to render_file('/etc/php/7.4/php.ini').with_content(/session.save_path=\/tmp\/sessions/)
  end
  ...
  %w(php7.4-apcu php7.4-apcu-bc php7.4-curl php7.4-mbstring).each do | pkg |
